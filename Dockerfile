...
RUN echo '#!/bin/sh'                                                                                         > /entrypoint/primer.sh && \
	...
	echo 'echo "      proxy_pass_request_headers    on;"                                     >> default.conf' >> /entrypoint/primer.sh && \
        ...
        echo 'echo "      proxy_set_header X-Forwarded-Proto \$scheme;"                          >> default.conf' >> /entrypoint/primer.sh && \
        echo 'echo "      proxy_set_header X-Forwarded-Protocol \$scheme;"                       >> default.conf' >> /entrypoint/primer.sh && \
        echo 'echo "      proxy_set_header X-Forwarded-Host \$host;"                             >> default.conf' >> /entrypoint/primer.sh && \
